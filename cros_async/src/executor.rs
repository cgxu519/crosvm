...
impl<T> ExecutableFuture<T> {
    ...
    pub fn new(future: Pin<Box<dyn Future<Output = T>>>) -> ExecutableFuture<T> {
        ExecutableFuture {
            future,
            needs_poll: Rc::new(AtomicBool::new(true)),
        }
    }
    ...
    fn poll(&mut self) -> Poll<T> {
        ...
        let waker = unsafe {
            let clone = self.needs_poll.clone();
...
pub(crate) trait FutureList {
    ...
    // Polls all futures that are ready. Returns the results if this list is completed running.
...
impl UnitFutures {
    ...
    pub fn poll_all(&mut self) {
        ...
        while i < self.futures.len() {
            ...
            let remove = if fut.needs_poll.swap(false, Ordering::Relaxed) {
                fut.poll().is_ready()
            } else {
                false
            };
